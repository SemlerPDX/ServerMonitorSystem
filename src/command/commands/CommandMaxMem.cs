...
namespace ServerMonitorSystem
{
    ...
    /// providing access to a method for setting a new <see cref="IConfig_Manager.MaxMem"/> value.
    /// <para>
    /// Dependencies: <see cref="IConfig_Manager"/>, <see cref="IConsole_Manager"/>
    /// </para>
    /// </summary>
    class CommandMaxMem : ICommands
    {
        /// <summary>
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="CommandMaxMem"/> class with the specified interfaces.
        /// </summary>
        ...
        public CommandMaxMem(IConfig_Manager configManager,
        ...
        public string Name { get; } = "maxmem";
        public string Usage { get; } = "maxmem <int>";
        public string Description { get; } = "Set maximum used RAM after which to issue Leak Alert (in megabytes, default 9216)";
        public bool ConfigSetting { get; } = true; // required true for commands changing any on-file config setting

        private const string DEFAULT_PROPERTY_DESIGNATION = "Maximum used memory";
        private const string DEFAULT_PROPERTY_DESCRIPTOR = "megabytes";
        private const string DEFAULT_PROPERTY_CHANGED = "maxmem"; // must match configManager property used below (lowercase) for command line overrides

        // Set the number range default values in the appropriate number type ex. private const float ...
        private const float DEFAULT_NUMBER_MIN = 1F;
        private const float DEFAULT_NUMBER_MAX = float.MaxValue;

        // Cast ConfigValue and NewValue as appropriate type referring to configManager property and an out of range default value
        private float ConfigValue { get => _configManager.MaxMem; set => _configManager.MaxMem = value; }
        private float NewValue { get; set; } = -1F;
        ...
        private bool SetNewValue()
        {
            ...
            {
                Console.WriteLine(" -invalid number out of range (min:{0})", DEFAULT_NUMBER_MIN, DEFAULT_NUMBER_MAX);
                return false;
            }
        }
        ...
        private bool GetNewValue(string[] args)
        {
            ...
            if (args.Length == 1)
            {
                ...
                Console.WriteLine("Enter a new {0} (in {1}) (min:{2} now:{4})",
